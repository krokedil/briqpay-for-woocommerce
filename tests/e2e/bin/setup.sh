#!/bin/bash
ls -la
wp core install --url=http://localhost:8000 --title="Briqpay Checkout" --admin_user=admin --admin_password=password --admin_email=info@example.com --path=/var/www/html --skip-email;
...
wp plugin activate briqpay-for-woocommerce
...
wp option update woocommerce_kco_settings < wp-content/plugins/briqpay-for-woocommerce/tests/e2e/bin/settings.json  --format=json
